...
import { canAuthenticated, isAdmin, isAuthenticated } from "./auth";
...
export default class Book {
  ...
  register() {
    ...
    this.router.get("/:bookId", canAuthenticated, this.getById);
  ...
  async getById(req: Request, res: Response) {
    ...
    if (Number.isNaN(bookId)) return res.status(400).json({ message: "invalid book id" });

    let book: any = await db.select(bookAllFields).from(books).where(eq(books.id, bookId));
    if (book.length < 1) return res.status(404).json({ message: "not found" });

    book = await addCategoriesToBook(book[0]);

    const user = req.user as User | undefined;
    if (user) {
      const result = await db.query.usersToBooks.findFirst({
        where: and(eq(usersToBooks.userId, user.id), eq(usersToBooks.bookId, book.id)),
      });
      book.favorited = !!result;
    }
  ...
  async favorite(req: Request, res: Response) {
    ...
    await db.insert(usersToBooks).values({ bookId: bookId, userId: user.id }).onConflictDoNothing();
...
function getDataFromBody(body: any) {
  ...
  return {
    ...
    stocksAvailable: stocksAvailable ?? "" ? Number(stocksAvailable) : 0,
